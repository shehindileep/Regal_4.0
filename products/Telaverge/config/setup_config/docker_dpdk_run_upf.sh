...
# UPF_LBU_EXT_DEV="0000:03:10.0,0000:03:10.4,0000:03:11.0,0000:03:11.4"
UPF_LBU_EXT_DEV="{{lbu_ext_dev}}"
UPF_LBU_INT_DEV="{{lbu_int_dev}}"
UPF_FPU_DEV="{{fbu_dev}}"
...
docker run -tid --cap-add ALL --cpuset-cpus="$UPF_SMU_CPUS" -e "UPF_RUNCONFIG=/opt/upf/config/smu/smu_docker.ini" -e "PATH=$PATH:/opt/upf/bin" --net=host \
                                -w "/opt/upf" \
                                -v $UPF_CONFIG_PATH:/opt/upf/config:z \
                                -v $UPF_BIN_PATH:/opt/upf/bin:z \
                                -v $UPF_LIB_PATH:/opt/upf/lib:z \
                                -v $UPF_SCRIPT_PATH:/opt/upf/script:z \
