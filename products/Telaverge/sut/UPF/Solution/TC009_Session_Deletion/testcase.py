...
Steps:
    ...
    21. Copy pcap file from unicorn node and get session ID and send it with Unicorn Modification and Deletion request.
    ...
    33. Validate uplink data transfer between gNodeb and DN nodes. 
    34. Deleting SMF context request sending from Unicorn simultaor.   
    35. PFCP Session Deletion Request sent from SMF to UPF.   
    36. PFCP Session Deletion Response sent from UPF to SMF.   
    37. Deleting SMF context request sent from Unicorn simultaor.
    38. Capture packets in DN and gNodeB nodes.
    39. Start DN and gNodeB simulator.
    40. gNodeB simulator will send a packet towards UPF.
    41. UPF will not receive packet because session was deleted and will not send any packet towards gNodeB.
    42. UPF will not send any packets towards DN.
    43. Stop capturing packets DN and gNodeB nodes.
    45. Copy pcap file from DN and gNodeB.
    46. Validate zero data transfer between gNodeb and DN nodes. 
    47. Validate PFCP Associtaion Setup, Session Estashblishment, Session Modification and Session Deletion packets.
"""
...
class UPFTest():
    ...
    def setup_scripts_and_manage_services(self):
        ...
        self._log.info("Copied pcap file from Unicorn to get Session ID to perform Session Modification and Deletion.")
        ...
        self.start_data_exchange_between_gnodeb_and_dn()
        self._log.info("Copying pcap file from SMF node and validationg pcap traces.")
        self.smf_helper_obj.validate_session_pcap_file(UPFConstants.SESSION_MODIFICATION)
        self._log.info("PFCP Session Modification Response validated successfully.")
        self.validate_gnodeb_and_dn_data_exchange(UPFConstants.SESSION_MODIFICATION)
        self._log.info("Upllink data transfer validated successfully between gNodeB and DN.")
        self.unicorn_helper_obj.setup_real_node_script(False, UPFConstants.SESSION_DELETION, session_uuid)
        self._log.info("Copied Session Deletion Request json file to Unicorn.")
        self.unicorn_helper_obj.start_tc_execution(UPFConstants.UPF_DELETION, UPFConstants.SOLUTION_DELETION, UPFConstants.SESSION_DELETION)
        self._log.info("Delete SMF context sent from AMF simultaor")
        self._log.info("PFCP Session Deletion Request sent from SMF to UPF.")
        self.start_data_exchange_between_gnodeb_and_dn()
        ...
        self._log.info("Copying pcap file from SMF node and validationg pcap traces.")
        self.smf_helper_obj.validate_real_node_pcap(UPFConstants.SESSION_DELETION)
        self._log.info("PFCP Session Deletion Response validated successfully.")
        self.validate_gnodeb_and_dn_data_exchange(UPFConstants.SESSION_DELETION)
        self._log.info("Zero Data transfer validated between gNodeB and DN.")
        self._log.debug("<")

    def validate_gnodeb_and_dn_data_exchange(self, operation=None):
        ...
        if operation == UPFConstants.SESSION_DELETION:
            self.dn_helper_obj.validate_dn_pcap_after_session_deletion()
            self.gnb_helper_obj.validate_gnodeb_pcap_after_session_modification_and_deletion(UPFConstants.SESSION_DELETION)
        elif operation == UPFConstants.SESSION_MODIFICATION:
