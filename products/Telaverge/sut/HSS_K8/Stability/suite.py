...
import regal_lib.corelib.custom_exception as CommonException
...
def update_resource_till_machine_stops(node_name, total_wait_time=100, intervals=4):
    """ Method to update resources till machine stops
    ...
    """
    log.debug(">")
    end_time = time.time() + Constants.UPDATE_RESOURCE_TIMEOUT
    while True:
        try:
            update_resource_state_with_intervals(total_wait_time, intervals)
        except Exception as err:
            log.error("< Failed to update resource state")
            log.error(str(err))
            state = False
            while not state:
                state = cluster_helper_obj.is_machine_stopped(node_name)
                time.sleep(5)
        ...
        if time.time() > end_time:
            err_msg = "Timeout happened but machine {} didn't "\
                      "stop".format(node_name)
            raise exception.UpdateResourceTimeout(err_msg)
    log.debug("<")
...
def wait_for_pod_termination(node_name, micro_service_name):
    ...
    while count:
        ...
        for pod_obj in pod_objs:
            ...
            except CommonException.DeletedResource as ex:
